...
# ライセンス検証ダミー（Supabase連携準備用）
def verify_license(key_str: str) -> bool:
    if not key_str:
        return False
    clean_key = key_str.strip().upper()
    return clean_key.startswith("PRO-") or clean_key == "VIP2026"
...
with st.sidebar:
    ...
    st.caption("v2.5 // Cyber AI Localization Engine")
    st.markdown("---")

    # API設定
    st.markdown("#### 🔑 Gemini API 設定")
    gemini_key = st.text_input("Gemini API Key", type="password", placeholder="AIzaSy...", help="Google AI StudioのAPIキーを入力してください")
...
with col_l:
    st.markdown("""
        <div class="hero-container">
            <h1 style="color:#FFFFFF; font-family:'Orbitron', sans-serif; font-size:2.4rem; margin-bottom:4px;">
                TRANSLY <span style="color:#00F2FE;">PRO</span>
            </h1>
    ...
    # モード切り替えタブ
    tab1, tab2 = st.tabs(["🚀 MODE 1: フル動画・音声翻訳（PRO）", "⚡ MODE 2: クイック字幕・テキスト翻訳"])
        ...
        if not st.session_state.is_pro:
            st.markdown(
                ...
                <div class="cyber-lock-box">
                    <h3 style="color: #FF007F; margin-bottom: 8px; font-family: Orbitron;">🔒 MODE 1: PRO FEATURE LOCKED</h3>
    ...
    with tab2:
        ...
        source_text = st.text_area("翻訳元のテキストまたは字幕文", height=150, placeholder="ここにスクリプトや字幕を入力...")
        target_lang = st.selectbox("出力ターゲット言語", ["日本語", "英語 (US)", "簡体字中国語", "韓国語", "スペイン語"])
        
        if st.button("⚡ 高速AI翻訳を実行"):
            if not gemini_key:
